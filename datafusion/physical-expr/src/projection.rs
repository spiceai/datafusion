    ...
    pub fn project_statistics(
        ...
        for proj_expr in self.exprs.iter() {
            ...
            let col_stats = if let Some(col) = expr.downcast_ref::<Column>() {
                ...
                // `column_statistics_at` clones rather than takes, so a column
                // referenced more than once in `self.exprs` (e.g. `SELECT a, a AS a2`)
                // gets its statistics in every output slot, not just the first.
