...
impl Expr {
    ...
    pub fn infer_placeholder_types(self, schema: &DFSchema) -> Result<(Expr, bool)> {
        ...
        self.transform(|mut expr| {
            // Don't throw error early, only if Expr type is needed in specific `match` arm.
            let expr_type_result = expr.get_type(schema);
            match &mut expr {
                ...
                Expr::InList(InList {
                    ...
                    // Rewrite placeholders in the list based on the expr type
                    for item in list.iter_mut() {
                        rewrite_placeholder(item, expr.as_ref(), schema)?;
                    }
                    // Also rewrite the expr if it's a placeholder based on the list types
                    if let Some(first_non_placeholder) =
                        list.iter().find(|e| !matches!(e, Expr::Placeholder(_)))
                    {
                        rewrite_placeholder(
                            expr.as_mut(),
                ...
                Expr::Case(Case {
                    ...
                    let expr_type = expr_type_result?;
