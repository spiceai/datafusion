...
mod test {
    ...
    use crate::{
        ...
        ScalarUDF, ScalarUDFImpl, TableScan, Volatility, case, lit, placeholder,
    ...
    fn infer_placeholder_in_subquery() -> Result<()> {
        ...
        match inferred_expr {
            Expr::InSubquery(in_subquery) => match *in_subquery.expr {
                Expr::Placeholder(placeholder) => {
                    let inferred = placeholder.field.as_ref().unwrap();
                    assert_eq!(
                        inferred.data_type(),
                        &DataType::Int32,
            ...
            Expr::Between(between) => {
                ...
                match (*between.low, *between.high) {
                    (
                        ...
                        assert_eq!(
                            low_placeholder.field.as_ref().unwrap().data_type(),
                            &DataType::Float64,
                            "Placeholder {} should infer Float64",
                            low_placeholder.id
                        );
                        assert_eq!(
                            high_placeholder.field.as_ref().unwrap().data_type(),
                            &DataType::Float64,
                            "Placeholder {} should infer Float64",
                            high_placeholder.id
                        );
    ...
    fn infer_placeholder_cast_and_try_cast() {
        ...
        match inferred_expr {
            Expr::Cast(cast) => match *cast.expr {
                Expr::Placeholder(placeholder) => {
                    ...
                    assert_eq!(cast.field.data_type(), &DataType::Int32);
                }
        ...
        match inferred_expr {
            Expr::TryCast(try_cast) => match *try_cast.expr {
                Expr::Placeholder(placeholder) => {
                    ...
                    assert_eq!(try_cast.field.data_type(), &DataType::Float64);
                }
        ...
        match inferred_expr {
            Expr::Cast(cast) => {
                assert_eq!(cast.field.data_type(), &DataType::Float64);
                match *cast.expr {
                    Expr::TryCast(try_cast) => {
                        assert_eq!(try_cast.field.data_type(), &DataType::Int32);
                        match *try_cast.expr {
                            Expr::Placeholder(placeholder) => {
                                assert_eq!(
                                    placeholder.field.as_ref().unwrap().data_type(),
