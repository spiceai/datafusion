...
impl ListingTable {
    ...
    async fn do_collect_statistics_and_ordering(
        ...
        // stats computed under a different file schema. `statistics_cache`
        // additionally withholds the cache entirely for an anonymous,
        // explicitly-schema'd read (Spice extension): such reads can reuse the
        // same file path under different logical schemas, and the fingerprint
        // check alone cannot distinguish "file changed" from "same file, read
        // under a different declared schema".
