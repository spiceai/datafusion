...
pub struct TableSchema {
    ...
    /// partition columns and *before* any virtual columns in
    /// [`Self::table_schema`].
    ...
    /// file, partition, and metadata columns (layout: `[file, partition,
    /// metadata, virtual]`).
    virtual_columns: Fields,

    /// The complete table schema: file_schema columns, followed by partition
    /// columns, followed by metadata columns, followed by virtual columns.
    ///
    /// This is pre-computed during construction by concatenating the four
...
impl TableSchema {
    ...
    /// Compute the complete table schema, and the schema without virtual
    /// columns, by concatenating the file schema, partition columns, metadata
    /// columns, and virtual columns (in that order).
    fn compute_schemas(
        file_schema: &SchemaRef,
        table_partition_cols: &Fields,
        ...
        if virtual_columns.is_empty() {
            (Arc::clone(&without_virtual), without_virtual)
        } else {
    ...
    /// replacing any existing ones. Existing metadata and virtual columns are
    /// preserved.
    #[deprecated(
        since = "55.0.0",
    ...
    pub fn with_table_partition_cols(self, partition_cols: Vec<FieldRef>) -> Self {
        TableSchemaBuilder::new(self.file_schema)
            ...
            .with_metadata_cols((*self.metadata_cols).clone())
    ...
    /// columns and *before* any virtual columns. They expose per-file
    /// [`object_store::ObjectMeta`] information such as `_location`, `_size`,
    /// and `_last_modified`.
    pub fn with_metadata_cols(mut self, metadata_cols: Vec<MetadataColumn>) -> Self {
        self.metadata_cols = Arc::new(metadata_cols);
        let (table_schema, schema_without_virtual_columns) = Self::compute_schemas(
            &self.file_schema,
        ...
        self.schema_without_virtual_columns = schema_without_virtual_columns;
        self
    }
    ...
    /// before any virtual columns, and are populated from each file's
    /// [`object_store::ObjectMeta`] during query execution.
    ...
    /// Get the full table schema (file schema + partition columns + metadata
    /// columns + virtual columns).
    ///
    /// This is the complete schema that will be seen by queries. Fields appear
    /// in the order: file columns, partition columns, metadata columns,
    /// virtual columns.
    ...
    /// file reader: file columns plus partition columns plus metadata columns,
    /// excluding virtual columns.
    ///
...
impl TableSchemaBuilder {
    ...
    /// partition, metadata, or virtual columns yet.
    pub fn new(file_schema: SchemaRef) -> Self {
        Self {
            file_schema,
            table_partition_cols: Fields::empty(),
            metadata_cols: Arc::new(Vec::new()),
    ...
    /// Set the metadata columns (Spice extension), replacing any previously
    /// set.
    ///
    /// Metadata columns expose per-file [`object_store::ObjectMeta`]
    /// information (such as `_location`, `_size`, and `_last_modified`). They
    /// are appended to the table schema after the partition columns and
    /// before any virtual columns.
    pub fn with_metadata_cols(mut self, metadata_cols: Vec<MetadataColumn>) -> Self {
        self.metadata_cols = Arc::new(metadata_cols);
        self
    }
    ...
    /// `file + partition + metadata + virtual` schema once.
        ...
        debug_assert!(
            self.virtual_columns.iter().enumerate().all(|(i, v)| {
                ...
                !self.file_schema.fields().iter().any(|f| f.name() == name)
                    ...
                    && !self.metadata_cols.iter().any(|m| m.field().name() == name)
                    && !self.virtual_columns[..i].iter().any(|w| w.name() == name)
            }),
            "virtual column name collides with an existing file, partition, metadata, or virtual column"
        );

        let (table_schema, schema_without_virtual_columns) = TableSchema::compute_schemas(
            &self.file_schema,
            &self.table_partition_cols,
            &self.metadata_cols,
            &self.virtual_columns,
